...
function buildQuoteHTML(d){
  ...
  <div class="qp-cover" style="${coverUrl?`background:url('${coverUrl}') center/cover no-repeat`:'background:linear-gradient(160deg,#0D2B1E 0%,#0A1A12 50%,#0D120F 100%)'}">
    ...
    <div style="position:absolute;bottom:0;left:0;right:0;z-index:2;background:${th.grad};padding:12px 36px ${window._unsplashCredit?'18px':'12px'};display:flex;flex-direction:column;gap:0">
      ...
      ${window._unsplashCredit?(()=>{const _uc=window._unsplashCredit;const _pb=_uc.link||'https://unsplash.com';const _pu=_pb+(_pb.includes('?')?'&':'?')+'utm_source=ermix&utm_medium=referral';return`<div style="text-align:right;margin-top:6px;font-size:6.5px;color:rgba(255,255,255,.35)">Foto de <a href="https://unsplash.com/@${_uc.username}?utm_source=ermix&utm_medium=referral" target="_blank" style="color:rgba(255,255,255,.5);text-decoration:underline">${_uc.name}</a> en <a href="${_pu}" target="_blank" style="color:rgba(255,255,255,.5);text-decoration:underline">Unsplash</a></div>`})():''}
    </div>
  </div>`;
