...
function buildQuoteHTML(d){
  ...
  <div class="qp-cover" style="${coverUrl?`background:url('${coverUrl}') center/cover no-repeat`:'background:linear-gradient(160deg,#0D2B1E 0%,#0A1A12 50%,#0D120F 100%)'}">
    ...
    ${window._unsplashCredit?`<div style="position:absolute;top:8px;right:12px;z-index:3;font-size:7.5px;color:rgba(255,255,255,.4);background:rgba(0,0,0,.25);padding:3px 8px;border-radius:10px;backdrop-filter:blur(4px)">Foto de <a href="https://unsplash.com/@${window._unsplashCredit.username}?utm_source=ermix&utm_medium=referral" target="_blank" style="color:rgba(255,255,255,.55);text-decoration:underline">${window._unsplashCredit.name}</a> en <a href="https://unsplash.com/?utm_source=ermix&utm_medium=referral" target="_blank" style="color:rgba(255,255,255,.55);text-decoration:underline">Unsplash</a></div>`:''}
  </div>`;
  ...
  if(allDates.length>=2){
    ...
    if(rows.length>0&&rows.length<=60){
      H+=`<div class="qp-page">${darkHd(`ITINERARIO · ${(vi.destino||'').toUpperCase()}${vi.salida?' · '+fd(vi.salida):''}${vi.regreso?' – '+fd(vi.regreso):''}`,`Día a Día`,vi.noches?vi.noches+' noches'+(vi.destino?' en '+vi.destino:''):'')}
  ...
  // RESUMEN ECONÓMICO
  // ════════════════════════════════════════════════
  H+=`<div class="qp-page">${darkHd('RESUMEN ECONÓMICO','Condiciones y Precios',pr.validez?'Validez: '+pr.validez:'Válido por 24 horas')}<div style="padding:20px 32px">`;
  const prLines=[
