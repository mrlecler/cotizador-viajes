...
function buildQuoteHTML(d){
  ...
  <div class="qp-cover" style="${coverUrl?`background:url('${coverUrl}') center/cover no-repeat`:'background:linear-gradient(160deg,#0D2B1E 0%,#0A1A12 50%,#0D120F 100%)'}">
    ...
    ${window._unsplashCredit?(()=>{const _uc=window._unsplashCredit;const _pb=_uc.link||'https://unsplash.com';const _pu=_pb+(_pb.includes('?')?'&':'?')+'utm_source=ermix&utm_medium=referral';return`<div style="position:absolute;bottom:6px;right:8px;z-index:4;font-size:7px;color:rgba(255,255,255,.6);background:rgba(0,0,0,.42);padding:3px 8px;border-radius:8px;backdrop-filter:blur(6px);line-height:1.4">Foto de <a href="https://unsplash.com/@${_uc.username}?utm_source=ermix&utm_medium=referral" target="_blank" style="color:rgba(255,255,255,.85);text-decoration:underline">${_uc.name}</a> en <a href="${_pu}" target="_blank" style="color:rgba(255,255,255,.85);text-decoration:underline">Unsplash</a></div>`})():''}
  </div>`;
