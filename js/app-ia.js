async function processAI(){
  ...
  try{
    ...
    if(data.error){
      ...
      if(ec==='overloaded_error')throw new Error('⚠️ Los servidores de Anthropic están sobrecargados. Intentá nuevamente en unos minutos.');
      ...
      if(data.error.message?.toLowerCase().includes('overload'))throw new Error('⚠️ Los servidores de Anthropic están sobrecargados. Intentá en unos minutos.');
    ...
    if(msg.includes('429')||msg.includes('Too Many'))msg='⏱️ Límite de solicitudes alcanzado. Esperá un momento.';
    if(msg.includes('402')||msg.toLowerCase().includes('billing'))msg='💳 Créditos de Anthropic agotados. Recargá en console.anthropic.com.';
    showIAErr(msg);
  }
  finally{btn.disabled=false;btn.innerHTML='✨ Procesar con IA';}
}
